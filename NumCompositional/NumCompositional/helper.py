...
def setup_fluids(fluids):
    properties = properties_df.ix[fluids['molecule']]
    fluids['mw'] = np.array(properties['weight']) * 0.00220462
    fluids['pc'] = np.array(properties['pressure']) * 14.6959
    fluids['tc'] = np.array(properties['temperature']) * 1.8
    fluids['w'] = np.array(properties['accentric'])
    fluids['oa'] = np.array(properties['omega_a'])
    fluids['ob'] = np.array(properties['omega_b'])
