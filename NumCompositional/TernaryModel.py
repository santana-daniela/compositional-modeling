...
def create_composition_data(molecules, pressure, temperature, points, scale):
    rows = len(points)
    ...
    for line in grid:
        pnt = 0
        mixes = solve_line(line, molecules, pressure, temperature)
        last_mix = None
        for mix in mixes:
...
def solve_line(line, molecules, pressure, temperature):
    ...
    #solve end points
    if count == 1:
        mix = solve_point(line[0], molecules, pressure, temperature)
        return [mix]
    mixes[0] = solve_point(line[0], molecules, pressure, temperature)
    mixes[max_p] = solve_point(line[max_p], molecules, pressure, temperature)
...
def create_ternary(molecules, pressure, temperature, amt=30, heatmap=False, tie_lines=False, specific_zFracs=[], finish_curve=False):
    scale = 2
    ...
    mixed_data = []
    if heatmap:
        mixed_data = create_composition_data(molecules, pressure, temperature, points, scale)
        values = mixed_data[0]
...
#ternary_plot = create_ternary(molecules, amt=amount, heatmap=show_heat, tie_lines=show_tie,
